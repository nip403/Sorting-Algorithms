        ...
        for index,item in enumerate(array):
            if self.bars:
                if index in highlighted:
                    pygame.draw.rect(self.surface,self.sorting_colour,(index*self.thickness,self.s[1]-(item/len(array)*self.s[1]),math.ceil(self.thickness),item/len(array)*self.s[1]),0)
                elif index in self.green:
                    pygame.draw.rect(self.surface,self.done_colour,(index*self.thickness,self.s[1]-(item/len(array)*self.s[1]),math.ceil(self.thickness),item/len(array)*self.s[1]),0)
                else:
                    pygame.draw.rect(self.surface,self.base_colour,(index*self.thickness,self.s[1]-(item/len(array)*self.s[1]),math.ceil(self.thickness),item/len(array)*self.s[1]),0)
